...
void downloadWithCurl(const std::string& url, Callback fn, void* data) {
  ...
  if (CURLE_OK != curl_easy_perform(curl)) {
    ...
    throw std::runtime_error("Cannot download " + url);
  }
  curl_easy_cleanup(curl);
}

template <typename T>
void downloadToStreamWithCurl(const std::string& url, T* stream) {
